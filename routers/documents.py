...
async def add_document(doc: Document, user: User = Depends(get_current_active_user)):
    if doc.id in documents:
        print("********* Rejecting due to duplicate ID")
    ...
    if documents.get_by_path(doc.path):
        print("********* Rejecting due to duplicate path")
...
async def delete_document(doc_id: str, user: User = Depends(get_current_active_user)):
