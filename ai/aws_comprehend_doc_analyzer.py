...
def usage_demo():
	...
	print("Detecting languages.".center(88, '*'))
	languages = comp_detect.detect_languages(sample_text)
	pprint(languages)
	lang_code = languages[0]['LanguageCode']
	"""
	print("Detecting entities.".center(88, '*'))
	entities = comp_detect.detect_entities(sample_text, lang_code)
	print(f"The first {demo_size} are:")
	pprint(entities[:demo_size])

	print("Detecting key phrases.".center(88, '*'))
	phrases = comp_detect.detect_key_phrases(sample_text, lang_code)
	print(f"The first {demo_size} are:")
	pprint(phrases[:demo_size])
	"""
	print("Detecting personally identifiable information (PII).".center(88, '*'))
	pii_entities = comp_detect.detect_pii(sample_text, lang_code)
	print(f"The first {demo_size} are:")
	entities = {}
	for entity in pii_entities[:demo_size]:
		type_name = entity['Type']
		type_text = sample_text[entity['BeginOffset']:entity['EndOffset']]
		print(f"Type: {type_name}, Text: {type_text}, Score: {entity['Score']}")
		if type_name not in entities:
			entities[type_name] = []
		entities[type_name].append(type_text)
	print("Account:     ", entities['BANK_ACCOUNT_NUMBER'][0])
	print("Name:        ", entities['NAME'][0])
	print("Date:        ", entities['DATE_TIME'][3])
	# pprint(pii_entities[:demo_size])
	"""
	print("Detecting sentiment.".center(88, '*'))
	sentiment = comp_detect.detect_sentiment(sample_text, lang_code)
	print(f"Sentiment: {sentiment['Sentiment']}")
	print("SentimentScore:")
	pprint(sentiment['SentimentScore'])

	print("Detecting syntax elements.".center(88, '*'))
	syntax_tokens = comp_detect.detect_syntax(sample_text, lang_code)
	print(f"The first {demo_size} are:")
	pprint(syntax_tokens[:demo_size])
	"""
	print("Thanks for watching!".center(88, '*'))
	print('-'*88)
